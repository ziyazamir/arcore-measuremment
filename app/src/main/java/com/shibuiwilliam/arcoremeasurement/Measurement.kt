...
class Measurement : AppCompatActivity(), Scene.OnUpdateListener {
    ...
    private fun isTimestampTooNew(timestamp: Long): Boolean {
        // Define a threshold for what is considered too new (e.g., 100 milliseconds)
        val threshold = 100 * 1_000_000L  // Convert milliseconds to nanoseconds

        // Compare the timestamp with the current time
        val currentTime = System.nanoTime()
        val timeDifference = currentTime - timestamp

        // If the time difference is less than the threshold, it's considered too new
    ...
    override fun onUpdate(frameTime: FrameTime) {
        ...
        if (frame != null) {
            ...
            // Check if the timestamp is too new
            if (isTimestampTooNew(currentTimestamp)) {
                Log.e(TAG, "Timestamp is too new. Skipping update.")
                return
            }

            // Perform your AR-related operations here
            when(distanceMode) {
                distanceModeArrayList[0] -> {
